...
//通过传入进程名，遍历PID来获取进程的Eprocess结构

ULONG_PTR FindProcessName(PWCHAR ProcessName)
{
	...
	for (int i = 8; i < 1000000; i += 4)
	{
		...
		NTSTATUS st = PsLookupProcessByProcessId((HANDLE)i, &TempProcess); //用这个函数要将引用次数减1
		if (!NT_SUCCESS(st))
		{
			continue;
		}
		...
		//DbgPrintEx(77, 0, "[DBG]baseName Handle Address = %S\r\n", Path->Buffer);
		PWCH baseName = wcsrchr(Path->Buffer, L'\\');

		if (baseName)
		{
			baseName += 1;
			//DbgPrintEx(77, 0, "[DBG]baseName = %S\r\n", baseName);
			if (_wcsicmp(baseName, ProcessName) == 0)
			{
				//DbgBreakPoint();
				DbgPrintEx(77, 0, "[DBG]ProcessName = %S\r\n", baseName);
				process = TempProcess;
		...
		if (process)
		{
			...
			DbgPrintEx(77, 0, "[DBG]EPROCESS Handle Address = %llx\r\n", process);
			break;
		}
	}
